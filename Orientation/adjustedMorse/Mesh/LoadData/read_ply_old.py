...
def read_ply(filename, quality_index, vertices_dict, edges_dict, faces_dict, links_dict):
    ...
    for vindex, pt in enumerate(rawdata['vertex']):
        vert = Vertex(x=pt[0], y=pt[1], z=pt[2], index=vindex, quality=pt[quality_index], nx=pt[8], ny=pt[9], nz=pt[10], theta=np.arccos(pt[10]), phi=np.arcsin(pt[8]/np.cos(np.arccos(pt[10]))))
